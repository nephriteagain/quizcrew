...
import { openAiImagePromptSchema } from "./schema/ai.js";
config();
...
fastify.post("/quiz/from-images", {
    ...
    fastify.log.info(`[${requestId}] Processing ${images.length} image(s) for quiz type: ${type}`);
    ...
    const { outputName, description, structuredSchema } = getImageInstructions(type, fastify, requestId);
    ...
    try {
        ...
        fastify.log.info(`[${requestId}] Generated quiz with ${response.output || "unknown"} questions`);
